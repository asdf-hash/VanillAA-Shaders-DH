...
void main() {
    ...
    // Overdraw prevention: discard DH terrain within vanilla render distance
    // This creates a sphere around the player where DH doesn't render
    float dist = length(viewPos);
    float overdrawRadius = far * 0.85; // Start DH terrain at 85% of vanilla render distance
    
    if (dist < overdrawRadius) {
        discard;
    }
    
    // Smooth transition at the edge to avoid hard cutoff
    float transitionWidth = far * 0.15;
    float alpha = smoothstep(overdrawRadius, overdrawRadius + transitionWidth, dist);
    color.a *= alpha;
    ...
    // Optional: Add visual distinction for water/lava
    if (materialId == MAT_WATER) {
        color.rgb *= vec3(0.9, 0.95, 1.0); // Slight blue tint
    } else if (materialId == MAT_LAVA) {
        color.rgb *= vec3(1.0, 0.7, 0.5); // Orange tint
    }
