...
uniform float blindness, darknessFactor;
...
void main() {
    if(isEyeInWater != 0 || blindness != 0 || darknessFactor != 0){discard;}
    ...
    float overdrawRadius = far * DH_OVERDRAW;
    if (DH_OVERDRAW < 0.001){
        if (far <= 32){
            overdrawRadius = far*0.95f;
        }
        else if (far <= 160){
            overdrawRadius = far*0.7f;
        }
        else{
            overdrawRadius = far*0.7f;
        }
    ...
    if (DH_NOISE == 1 && materialId != DH_BLOCK_UNKNOWN) {
    ...
    if (DH_FOG_START == 0.0){
	minFogDistance = float(far);
    }
    float fogBlendValue = clamp((dist - minFogDistance) / (maxFogDistance - minFogDistance), 0.0, 1.0);
