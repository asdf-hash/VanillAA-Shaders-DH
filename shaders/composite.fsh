...
float calculateSSAO(vec2 uv, float depth, vec3 viewPos) {
    ...
    float radius = SSAO_RADIUS + max(logScale-8.6+max(fovY_degrees-110,0.0)*0.05,0.0)*1.2;
    float intensity = max(SSAO_INTENSITY - depthLinear * (0.0005),0.5);
...
void main()
{
    ...
    if (dhDepth < 1.0 && regularDepth >= 1.0 && SSAO_SAMPLES != 0) {
